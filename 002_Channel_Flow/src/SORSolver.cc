...
bool SORSolver::solve(StaggeredGrid & grid)
{
    ...
    {
        double temp = 0;
        double calcs = ((grid.p().operator()(i-1,j)-(2*grid.p().operator()(i,j))+grid.p().operator()(i+1,j))/(grid.dx()*grid.dx())) +
        ...
        {
            residual += ((res[it]*res[it])/(imax_*jmax_));
        }
