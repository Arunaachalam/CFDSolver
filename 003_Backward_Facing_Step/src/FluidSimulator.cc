...
for (int j = 1; j < solver().jmax()+1; j++)
{
    ...
    {
        grid().f().operator ()(i,j) = 0;
        grid().g().operator ()(i,j) = 0;
    }
}
}
...
void FluidSimulator::flagfill()
{
    grid_.fill(true);
    grid_.createRectangle(grid_.Rectx1(), grid_.Recty1(), grid_.Rectx2(), grid_.Recty2());
    grid_.createObstacleEnumField();
    grid_.imagewrite();
}

void FluidSimulator::correctBoundary()
{
    for (int i = 1; i < solver().imax(); i++)
    for (int j = 1; j < solver().jmax(); j++)
    {
